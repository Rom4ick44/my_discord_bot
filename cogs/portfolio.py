...
def get_rank_emoji(rank):
    """Возвращает кастомный эмодзи для указанного ранга (полная строка <:name:id>)"""
    emoji_map = {
        'Academy': EMOJI_ACADEMY,
        'Reed': EMOJI_REED,
        'Main': EMOJI_MAIN,
        'High': EMOJI_HIGH
    }
    return emoji_map.get(rank, "📁")  # fallback на юникод
...
async def refresh_portfolio_embed(channel):
    ...
    # Используем кастомный эмодзи для отображения ранга
    rank_display = f"{get_rank_emoji(rank)} {rank}" if rank else "Нет ранга"
...
# ---------- Функция для создания портфеля (вызывается из других модулей) ----------
async def create_portfolio_for_user(guild, member):
    """Создаёт портфель для указанного участника (используется при принятии заявки)."""
    ...
    # Формируем название канала: только ник (без дискриминатора и без эмодзи)
    ...
    # Используем кастомный эмодзи
    embed.add_field(name="Текущий Ранг", value=f"{get_rank_emoji(rank)} {rank}", inline=True)
...
class PortfolioActionSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        asyncio.create_task(self._process_action(interaction, action, channel, owner, current_rank, current_tier, pinned_by))

    async def _process_action(self, interaction, action, channel, owner, current_rank, current_tier, pinned_by):
        try:
            if action == "rank_up":
                if not owner:
                ...
                if current_rank == 'Academy':
                    await interaction.followup.send("❌ Это минимальный ранг.", ephemeral=True)
                    return
                rank_order = ['Academy', 'Reed', 'Main', 'High']
                ...
                await refresh_portfolio_embed(channel)
                await interaction.followup.send(f"✅ Ранг понижен до {prev_rank}.", ephemeral=True)

            elif action == "pin":
                db.update_portfolio_pinned(channel.id, interaction.user.id)
                await refresh_portfolio_embed(channel)
                await interaction.followup.send("✅ Вы закрепились за портфелем.", ephemeral=True)

            elif action == "unpin":
                db.update_portfolio_pinned(channel.id, None)
                await refresh_portfolio_embed(channel)
                await interaction.followup.send("✅ Вы открепились.", ephemeral=True)
...
class PromotionRequestModal(Modal, title="Запрос повышения"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send("✅ Запрос отправлен куратору.", ephemeral=True)
...
class GreenRequestModal(Modal, title="Запрос грина"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not channel:
            return await interaction.followup.send("❌ Канал для запросов грина не настроен.", ephemeral=True)
        ...
        await interaction.followup.send("✅ Запрос отправлен. Ожидайте решения.", ephemeral=True)
...
class GreenRequestView(View):
    ...
    async def grant_green(self, interaction: discord.Interaction):
        ...
        if not req_data:
            return await interaction.followup.send("❌ Запрос не найден.", ephemeral=True)
        ...
        await interaction.followup.send("✅ Грин отмечен как выданный.", ephemeral=True)
...
class CreatePortfolioView(View):
    ...
    async def create_button_callback(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            # Формируем название канала: только ник (без эмодзи)
...
class Portfolio(commands.Cog):
    ...
    async def on_member_update(self, before, after):
        ...
        if not channel:
            return
        # Обновляем название канала (только ник)
    ...
    async def fix_portfolio_names(self, ctx):
        """Переименовывает все существующие портфели в соответствии с текущим ником владельца (без эмодзи)."""
        ...
        for channel_id, owner_id, rank, tier, pinned_by, thread_rp_id, thread_gang_id, _ in portfolios:
            ...
            if not owner:
                continue
            # Формируем новое название (только ник)
            ...
            if channel.name != new_name:
                try:
                    ...
                    renamed += 1
                    await asyncio.sleep(0.5)
                    # Обновляем embed для этого канала
                    await refresh_portfolio_embed(channel)
                except Exception as e:
